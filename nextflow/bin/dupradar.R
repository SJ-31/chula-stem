...
if (sys.nframe() == 0) {
  ...
  parser <- add_option(parser, c("-t", "--threads"), type = "integer", help = "N threads to use", default = 1)
  ...
  if (args$strandedness == "reverse") {
    strandedness <- 2
  } else if (args$strandedness == "forward") {
    strandedness <- 1
  } else {
    strandedness <- 0
  }
  dm <- dupRadar::analyzeDuprates(
    args$input_bam, args$gtf,
    stranded = strandedness,
    paired = paired, threads = args$threads
  )
  write.table(dm, args$output, sep = "\t", quote = FALSE, row.names = FALSE)
  if (!is.null(args$box_plot)) {
    png(filename = args$box_plot)
    duprateExpBoxplot(dm)
    dev.off()
  }
  if (!is.null(args$density_plot)) {
    png(filename = args$density_plot)
